...
const generatePrompt = (data: BoatFormData): string => {
  ...
  return `Analyze the following used boat details to provide a realistic 'ballpark' trade-in value range...`; // The full prompt from the original geminiService.ts goes here. It has been truncated for brevity.
};
...
export default async function handler(req: any, res: any) {
  ...
  try {
    await loadHistoricalData(); // Ensure data is loaded on each request (with caching)
    ...
    const response = await ai.models.generateContent({
      ...
      config: {
        tools: [{googleSearch: {}}],
        systemInstruction: `You are a marine vehicle appraisal expert for Legend Boats...`, // The full system instruction from original geminiService.ts goes here.
        temperature: 0.1,
      },
    });
    ...
    const match = responseText.trim().match(fenceRegex);
    const jsonStr = (match && match[1]) ? match[1].trim() : responseText.trim();
